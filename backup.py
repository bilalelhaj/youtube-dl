...
for row in range(1, sheet.max_row + 1):
    lang = sheet['A' + str(row)].value
    url = sheet['B' + str(row)].value
    title_to_save = sheet['C' + str(row)].value
    upload_title = sheet['D' + str(row)].value
    thumbnail = sheet['E' + str(row)].value
    full_part = sheet['F' + str(row)].value
    start = sheet['G' + str(row)].value
    complete_to_end = sheet['H' + str(row)].value
    end = sheet['I' + str(row)].value
...
def createImg():
    bidi = input("Title for thumbnail: ")
...
def main():
    url = input("URL: ")
    title = input("Set your title for the video to save it: ")
    title_x = title + "x"
    output = title_x + "x"
    output2 = output + "x"
    upload_T = input("Set your upload title: ")
    createImg()
    fVideo = input(
        "Do you want to download the full video or just a part of it?(full/part): ")
    if fVideo == "part":
        start = input("Start-Point: (Please in this format: (HH:MM:SS)): ")
        question2 = input(
            "Do you want the video from your start-point to the complete end ?(y/n): ")
        ...
        if question2 == "y":
            ...
            end = input("End: (Please in this format: (HH:MM:SS)): ")
